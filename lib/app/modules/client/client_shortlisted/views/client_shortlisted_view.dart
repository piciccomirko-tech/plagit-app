...
class ClientShortlistedView extends GetView<ClientShortlistedController> {
  ...
  Widget _employeeInSamePosition(String positionId) {
    final List<ShortList> employees =
        controller.shortlistController.getEmployeesBasedOnPosition(positionId);
    ...
    return Column(
      children: [
        Container(
          ...
          child: Row(
            children: [
              ((employee.employeeDetails?.profilePicture??"").isEmpty) || employee.employeeDetails?.profilePicture == "undefined"?
              Container(
                ...
                child: ClipRRect(
                    ...
                    child: Image.asset(MyAssets.employeeDefault, fit: BoxFit.fill)),
              ):
                            ...
                            children: [
                              ...
                              Row(
                                ...
                                children: [
                                  ...
                                  if (employee.employeeDetails?.certified !=
                                          null &&
                                      employee.employeeDetails?.certified ==
                                          true)
                                    Image.asset(MyAssets.certified,
                                        height: 30, width: 30),
                                  _rating(
                                      employee.employeeDetails?.rating ?? 0.0),
                                  const Spacer(),
                                  Obx(
                                    () => controller.shortlistController.getIcon(
                                        requestedDateList: <RequestDateModel>[],
                                        uniformMandatory: employee
                                                    .employeeDetails
                                                    ?.hasUniform ==
                                                true
                                            ? null
                          ...
                          InkWell(
                            ...
                            child: Container(
                              ...
                              child: Row(
                                children: [
                                  Text(
                                      ' ${employee.requestDateList?.calculateTotalDays()} ${MyStrings.daysSelected.tr}',
                          ...
                          if (employee.employeeDetails?.hasUniform == true)
                            InkWell(
                              onTap: () => controller.onUniformClick(
                                  positionId:
                                      employee.employeeDetails?.positionId ??
                                          '',
                                  shortListId: employee.sId ?? '',
                                  requestDateList:
                                      employee.requestDateList ?? []),
                              child: Container(
                                padding: const EdgeInsets.symmetric(
                                    horizontal: 10.0, vertical: 3.0),
                                decoration: BoxDecoration(
                            ...
                            // Text("${employee.employeeDetails?.name}"),
                            // Text("--${controller
                            //                 .shortlistController.selectedForHire.first.employeeDetails!.name}"),
                              ...
                              () => GestureDetector(
                                ...
                                child: Container(
                                  ...
                                  decoration: BoxDecoration(
                                    ...
                                    //         .contains(employee)
controller.shortlistController.selectedForHire.any((e) => e.employeeId == employee.employeeId)
...
controller.shortlistController.selectedForHire.any((e) => e.employeeId == employee.employeeId)
                              ...
                              InkWell(
                                  onTap: () =>
                                      Get.toNamed(Routes.calender, arguments: [
                                        employee.employeeId ?? '',
                                        employee.sId ?? '',
                                        employee.employeeDetails?.hasUniform ==
                                                true
                                            ? null
                                            : false
                                      ]),
        ...
        Container(
            ...
            width: 320.w,
            decoration: BoxDecoration(
                borderRadius: BorderRadius.circular(0.0).copyWith(
              ...
              children: [
                Text(
                    (employee.requestDateList ?? []).isNotEmpty
                        ? '${MyStrings.totalEstimatedAmount.tr}: ${Utils.getCurrencySymbol()}${employee.requestDateList?.calculateTotalHourlyRate(hourlyRate: employee.employeeDetails?.hourlyRate ?? 0.0).toStringAsFixed(2)}'
                        : '${MyStrings.salaryRange.tr}: ${Utils.getCurrencySymbol()}${employee.employeeDetails?.hourlyRate!.toStringAsFixed(2) ?? '0.00'}',
                    style: MyColors.white.medium12),
                SizedBox(height: 5,),
                Text('${MyStrings.perDayPlatformFee.tr}: \$5', style: MyColors.white.medium12),
                SizedBox(height: 5,),
                Text('${MyStrings.totalPlatformFee.tr}: \$${(employee.requestDateList ?? []).calculateTotalDays()*5}', style: MyColors.white.medium12),
              ],
            ))
      ],
    );
          ...
          child: CustomBottomBar(
            child: CustomButtons.button(
              ...
              text: controller.shortlistController.selectedForHire.isEmpty ||
                  ...
                  : "${MyStrings.book.tr} (${controller.shortlistController.selectedForHire.length}) ${MyStrings.candidate.tr}",
