...
import 'package:mh/app/modules/client/restaurant_location/models/google_auto_complete_search_model.dart';
import 'package:mh/app/modules/client/restaurant_location/models/lat_lng_model.dart';
import 'package:mh/app/modules/common_modules/auth/register/controllers/register_controller.dart';
...
class RestaurantLocationController extends GetxController {
  ...
  RxList<GoogleAutoCompleteSearchModel> googleAutoCompleteSearchList =
      <GoogleAutoCompleteSearchModel>[].obs;
  ...
  @override
  void onInit() {
    LocationController.getBytesFromAsset(
  ...
  void onConfirmPressed() {
    if (Get.isRegistered<RegisterController>()) {
      ...
      if (Get.arguments == "client") {
        registerController.restaurantLat = latLng.value.latitude;
        registerController.restaurantLong = latLng.value.longitude;
        registerController.restaurantAddressFromMap.value =
            locationText.value.trim();
        registerController.tecClientAddress.text = locationText.value.trim();
      } else {
        registerController.employeeLat = latLng.value.latitude;
        registerController.employeeLong = latLng.value.longitude;
        registerController.employeeAddressFromMap.value =
            locationText.value.trim();
        registerController.tecEmployeeAddress.text = locationText.value.trim();
      }
    } else if (Get.isRegistered<ClientEditProfileController>()) {
      final ClientEditProfileController profileController = Get.find();

      profileController.restaurantLat = latLng.value.latitude;
      profileController.restaurantLong = latLng.value.longitude;
      profileController.restaurantAddressFromMap.value =
          locationText.value.trim();
      profileController.tecRestaurantAddress.text = locationText.value.trim();
    } else if (Get.arguments == "employeeEditProfile" &&
        Get.isRegistered<EmployeeEditProfileController>()) {
      Get.find<EmployeeEditProfileController>().tecLocation.text =
          locationText.value.trim();
    }
  ...
  void onCameraIdle() {
    ...
    if (goToOnMapIdle == true) {
      locationText.value = MyStrings.fetchingAddress.tr;
      getAddressFromLatLng();
    }
  }
  ...
  void _getCurrentLocation() {
    LocationController.determinePosition()
        .then((Either<CustomError, Position> value) {
    ...
    markersList.add(
      Marker(
        infoWindow: InfoWindow(
            title: MyStrings.welcomeBack.tr,
            snippet: MyStrings.moveMarkerToLocation.tr),
  ...
  void onMapCreated(GoogleMapController c) async {
    String value = await DefaultAssetBundle.of(Get.context!)
        .loadString(MyAssets.customMapStyle);
    mapController = c;
  ...
  void getAddressFromLatLng() {
    LocationController.getAddressFromLatLngForGoogle(
            lat: latLng.value.latitude, lng: latLng.value.longitude)
  ...
  void getLatLngFromAddress({required String addressText}) {
    Utils.unFocus();
    goToOnMapIdle = false;
    LocationController.getLatLngFromAddressForGoogle(address: addressText)
        .then((LatLngModel? responseData) {
