...
class LiveLocationController extends GetxController {
  ...
  void loadMarkers({required LatLng employeePosition}) async {
    ...
    markersList.add( 
      Marker(
        infoWindow:  InfoWindow(title: MyStrings.yourLocation.tr, snippet: MyStrings.keepPatienceUntilEmployeeArrives.tr),
        icon: BitmapDescriptor.bytes(locationIcon.value),
    ...
    markersList.add(
      Marker(
        infoWindow:  InfoWindow(title: MyStrings.employeeLocation.tr, snippet: MyStrings.trackHisLiveLocation.tr),
    ...
    ui
      ...
      codec.getNextFrame().then((ui.FrameInfo frameInfo) async {
        ...
        if (byteData != null) {
          ...
          completer.complete(BitmapDescriptor.bytes(resizedBytes));
        } else {
  ...
  void calculatePolylinePoints(
    ...
    try {
      ...
      PolylineResult result = await polylinePoints
          .getRouteBetweenCoordinates(request: PolylineRequest(origin: origin, destination: destination, mode: travelMode),googleApiKey: AppCredentials.googleMapKey);
      socketLocationModel.value.currentPosition = result.startAddress;
      socketLocationModel.value.totalEta = parseDurationInMinutes(durationText: result.durationTexts?.first ?? "");
      ...
      Utils.showSnackBar(
          message: "${MyStrings.noRouteFor.tr} ${travelMode.toString().split('.').last} ${MyStrings.betweenThesePoints.tr}", isTrue: false);
    }
  }
