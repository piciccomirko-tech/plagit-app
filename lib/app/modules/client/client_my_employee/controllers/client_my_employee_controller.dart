...
class ClientMyEmployeeController extends GetxController {
  ...
  void getAllHiredEmployees({bool? allEmployees}) async {
    ...
    response.fold((CustomError customError) {
      ...
      if (emp.status == "success" &&
          ...
          emp.details != null) {
        try {
          if ((emp.details?.result!.employee!.first.bookedDate ?? []).isEmpty) {
            employees.value = [];
          } else {
            employees.value = emp.details?.result?.employee ?? [];
            for (EmployeeModel i in employees) {
              i.employeeDetails?.distance =
                  (LocationController.calculateDistance(
                      targetLat:
                      double.parse(i.employeeDetails?.lat ?? "0.0"),
                      targetLong:
                      double.parse(i.employeeDetails?.long ?? "0.0"),
                      currentLat: double.parse(
                          appController.user.value.client?.lat ?? "0.0"),
                      //23.795455885215837,
                      currentLong: double.parse(
                          appController.user.value.client?.long ?? "0.0")
                    //90.40503904223443
                  ) /
                      1609.34)
                      .toStringAsFixed(2);
  ...
  void onCalenderClick({required List<RequestDateModel> bookedDateList}) {
    bookedDateList.sort((RequestDateModel a, RequestDateModel b) =>
  ...
  void calculatePreviousDates(
    ...
    for (var i in bookedDateList) {
      if (DateTime.parse(i.endDate ?? '').toString().substring(0, 10) ==
  ...
  void onRadioButtonTap(String value) {
    if (value.isNotEmpty) {
      ...
      getAllHiredEmployees(allEmployees: true);
    }
  }
  ...
  Future<void> _selectDate(BuildContext context) async {
    ...
    onDatePicked(picked ?? DateTime.now());
    getAllHiredEmployees();
  }
  ...
  void onPrevDatePressed({required String employeeId}) async {
    ...
    if (prevDateList.isNotEmpty) {
      prevDateList.sort((RequestDateModel a, RequestDateModel b) =>
          a.startDate!.compareTo(b.startDate!));
    }
    Get.bottomSheet(
  ...
  Future<void> _getPreviousDate({required String employeeId}) async {
    ...
    response.fold((CustomError customError) {
      ...
      if (emp.status == "success" &&
        ...
        prevDateList.value =
            emp.details!.result?.employee?.first.bookedDate ?? [];
    ...
    Get.find<SocketController>().socket?.on('location:move', (data) async {
      ...
      for (var i in employees) {
        if (i.employeeId == socketLocationModel.value.sender) {
          i.employeeDetails?.lat =
              (socketLocationModel.value.cords?.latitude ?? 0.0).toString();
          i.employeeDetails?.long =
              (socketLocationModel.value.cords?.longitude ?? 0.0).toString();

          i.employeeDetails?.distance = (LocationController.calculateDistance(
                      targetLat:
                          socketLocationModel.value.cords?.latitude ?? 0.0,
                      targetLong:
