...
import '../controllers/nearby_candidates_controller.dart';
...
class NearbyCandidatesLocationView extends GetView<NearbyCandidatesController> {
  const NearbyCandidatesLocationView({super.key});
    ...
    return Scaffold(
        ...
        appBar: CustomAppbar.appbar(
          ...
          actions: [
            ...
            Obx(() {
              int count =
                  Get.find<ClientHomeController>().notificationsController.unreadCount.value;
              ...
              child: Stack(
                children: [
                  ...
                  GoogleMap(
                    ...
                    markers: Set.from(Iterable.generate(
                        (controller.employees.value.users?.length ?? 0) + 1,
                            (index) {
                          if (index <
                              (controller.employees.value.users?.length ?? 0)) {
                            var employee = controller.employees.value.users?[index];
                            final position = LatLng(
                                double.parse(employee?.lat ?? "0"),
                                double.parse(employee?.long ?? "0"));
                            return Marker(
                              markerId: MarkerId('professional_${employee?.id}'),
                              position: position,
                              icon: BitmapDescriptor.fromBytes(
                                  controller.employeeLocationIcon.value),
                              onTap: () {
                                controller
                              ...
                              child: Padding(
                                ...
                                child: Container(
                                  ...
                                  child: Image.asset(
                                    ...
                                    color:   Theme.of(context).brightness == Brightness.light?MyColors.black :MyColors.l7B7B7B_dicon(context),
