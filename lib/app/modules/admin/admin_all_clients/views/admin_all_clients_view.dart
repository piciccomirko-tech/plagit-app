                  ...
                  Expanded(
                    child: ListView.builder(
                      ...
                      itemBuilder: (BuildContext context, int index) {
                        if (index ==
                                (controller.clients.value.users ?? []).length -
                                    1 &&
          ...
          Row(
            ...
            children: [
              ...
              Container(
                ...
                child: CircleAvatar(
                  radius: 25.w,
                  backgroundImage:user.profilePicture==null? AssetImage(MyAssets.clientDefault): NetworkImage((user.profilePicture ?? "").imageUrl),
                ),
              ),
              // Container(
              ...
              //     (index + 1).toString(),
              //     style:
              //         MyColors.l111111_dffffff(controller.context!).semiBold18,
              //   ),
              // ),
SizedBox(width: 10.w,),
              Expanded(
                                      ...
                                      child: GestureDetector(
                                        onTap: () {
                                          ...
                                          ClientId clientId = ClientId(
                                            ...
                                            profilePicture:
                                                user.profilePicture?.split('/')[0] ??
                                                    '',
                                            name: user.restaurantName ?? '',
                                          );
                                          log("profuile img url: ${user.profilePicture?.imageUrl ?? ''}");
                                          Get.toNamed(
                                            Routes.individualSocialFeeds,
                                            arguments: controller
                                                .clientIdToSocialUser(clientId
                                                    // }
                                                    ),
                                          );
                                        },
                                        child: _name(user.restaurantName ?? ""),
                                      )),
                                  Expanded(
                                      flex: 2,
                      ...
                      children: [
                        const CircleAvatar(
                            ...
                            child: Icon(Icons.add_road_rounded,
                                size: 15, color: MyColors.c_FFFFFF)),
                        SizedBox(width: 5.w),
                        Expanded(
                          child: Text(
                            user.restaurantAddress ??
                                MyStrings.noAddressFound.tr,
                            maxLines: 2,
  ...
  Widget _chat({required Employee user}) =>
      controller.isFetching.value && controller.isFetchingId.value == user.id
          ? const SizedBox(
              width: 20,
              height: 20,
              child: Center(
                      ...
                      child: Obx(
                        () {
                          Iterable<Map<String, dynamic>> result = controller
                              .adminHomeController.clientChatDetails
