...
class AdminAllEmployeesView extends GetView<AdminAllEmployeesController> {
                  ...
                  Expanded(
                    child: ListView.builder(
                      ...
                      itemBuilder: (BuildContext context, int index) {
                        if (index ==
                                (controller.employees.value.users ?? [])
                                        .length -
                                    1 &&
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Text(
            " ${MyStrings.candidates.tr}${MyStrings.areShowing.tr}",
            style: Get.width > 600
          ...
          const Spacer(),
          Obx(() => Visibility(
              visible: controller.nationalityDataLoad.value == true &&
                  controller.hourlyRateDataLoaded.value == true,
  ...
  Widget _employeeItem(Employee user) {
    return Container(
      ...
      child: InkWell(
        ...
        child: Stack(
          children: [
            // Text("lang: ${StorageHelper.getLanguage}"),
            if ( StorageHelper.getLanguage == "ar")
              Positioned(
                ...
                child: SizedBox(
                  ...
                  child: CustomButtons.button(
                    height: 28.w,
                    text:
                        "${Utils.getCurrencySymbol()}${user.hourlyRate?.toStringAsFixed(2) ?? 0} / hour",
              ...
              Positioned(
                ...
                bottom: 0,
                child: SizedBox(
                  width: Get.width > 600 ? 80.w : 122.w,
                  child: CustomButtons.button(
                    height: Get.width > 600 ? 16.h : 28.h,
                    text:
                        "${Utils.getCurrencySymbol()}${user.hourlyRate?.toStringAsFixed(2) ?? 0} / hour",
                    margin: EdgeInsets.zero,
            ...
            if (StorageHelper.getLanguage == "ar")
              Positioned(
                ...
                child: GestureDetector(
                  onTap: () =>
                      controller.onCalenderClick(employeeId: user.id ?? ''),
                  child: Image.asset(
                    MyAssets.calender2,
                    height: 20,
                    width: 20,
                ...
                Expanded(
                  child: Column(
                    children: [
                      ...
                      SizedBox(height: 8.h),
                      // Text(controller.getPositionLogo(user.positionId!)),
                      // Text(user.positionId!),
                      // Text("${Utils.getPositionName(user.positionId!)}"), 
                      Row(
                        children: [
                          _detailsItem(
                              tag: 'position',
                              controller.getPositionLogo(user.positionId!),
                              Utils.getPositionName(user.positionId!),
                              ""),
                      ...
                      Row(
                        children: [
                          _detailsItem(
                            ...
                            (user.nationality ?? 'TBA').length > 10
                                ?  '${(StorageHelper.getLanguage=='ar'?'...':'')} ${(user.nationality ?? 'TBA').substring(0, 9)} ${(StorageHelper.getLanguage=='ar'?'':'...')}'
  ...
  Widget _detailsItem(String icon, String title, String value,
          {required String tag}) =>
      Expanded(
        child: Row(
          children: [
            tag == 'position'
                ? SizedBox(
                    width: 14.w,
                    height: 14.w,
                    child: CustomNetworkImage(url: icon.uniformImageUrl))
                : Image.asset(
                    icon,
                    width: 14.w,
                    height: 14.w,
  ...
  Widget _activeStatus({required bool active}) => Container(
        ...
        decoration: BoxDecoration(
          ...
          color:
              ((active == true)) ? MyColors.c_00C92C_10 : MyColors.c_FF5029_10,
        ),
        child: Row(
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                color:
                    ((active == true)) ? MyColors.c_00C92C : MyColors.c_C92C1A,
              ),
            ),
            const SizedBox(width: 5),
            Text(((active == true)) ? MyStrings.active.tr : 'Inactive',
                style: MyColors.l111111_dwhite(controller.context!).medium11)
          ],
        ),
      );
          ...
          child: Stack(
            ...
            children: [
              ...
              Positioned(
                ...
                child: Obx(
                  () {
                    Iterable<Map<String, dynamic>> result = controller
                        .adminHomeController.employeeChatDetails
                    ...
                    return Padding(
                      ...
                      child: CustomBadge(
                          result.first["allAdmin_unread"].toString()),
                    );
                  },
                ),
              ),
