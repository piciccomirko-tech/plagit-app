...
class CalenderMonthWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final int firstDayWeekday =
        (DateTime(month.year, month.month, 1).weekday + 5) % 7 + 1;
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        if (currentDate.isBefore(DateTime.now()
        ...
        } else if ((controller.dateListModel.value.bookedDates ?? [])
            .containsDate(currentDate)) {
          textColor = Colors.red;
        } else if ((controller.dateListModel.value.pendingDates ?? [])
            .containsDate(currentDate)) {
          // if ((controller.dateListModel.value.pendingDates ?? []).containsDate(currentDate) &&((currentDate.isAfter(DateTime.now()) || controller.selectedDate.value == currentDate)) ) {
          textColor = Colors.amber;
        } else if ((controller.dateListModel.value.unavailableDates ?? [])
            .containsDate(currentDate)) {
          //  if ((controller.dateListModel.value.unavailableDates ?? []).containsDate(currentDate)  &&((currentDate.isAfter(DateTime.now()) || controller.selectedDate.value == currentDate))) {
          ...
          canTapDate = true;
        } else if (currentDate.toString().substring(0, 10) ==
            today.toString().substring(0, 10)) {
          borderColor = MyColors.c_C6A34F; // Today's date should be red
          canTapDate = true;
        } 
          ...
          borderColor = MyColors
              .c_C6A34F; // Today's date should have a specific border color
        ...
        return GestureDetector(
          onTap: canTapDate
              ...
              : canUpdateUnavailableDate
                  ? () => controller.onEmployeeUnavailableDateUpdate(
                      currentDate: currentDate)
                  : null,
          child: Obx(() {
            final bool isSelected =
                controller.selectedDates.contains(currentDate);
            final bool isDateInRange =
                Get.isRegistered<EmployeeHomeController>()
                    ? controller.isDateInSelectedRangeForEmployee(currentDate)
                    : controller.requestDateList.any(
                        (RequestDateModel dateRange) =>
                            controller.isDateInSelectedRangeForShortList(
                                currentDate, dateRange));

            if (isSelected == true) {
              containerColor = MyColors.c_C6A34F;
              textColor =
                  MyColors.white; // Change container color for selected dates
              ...
              child: Text(
                ...
                style: TextStyle(
                    ...
                    fontSize: Get.width > 600
                        ? 20
                        : 13), // Text color based on textColor variable
              ),
            );
          }),
        );
