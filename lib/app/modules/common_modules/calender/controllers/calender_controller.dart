...
class CalendarController extends GetxController {
  ...
  void onDateClick({required DateTime currentDate}) {
    if (appController.user.value.isClient) {
    ...
    _apiHelper.getCalenderData(employeeId: employeeId).then((Either<CustomError, CalenderModel> responseData) {
      ...
      responseData.fold((CustomError customError) {
        ...
        if (response.status == "success" && response.statusCode == 200 && response.dateList != null) {
          ...
          log("calender date list:  booked: ${dateListModel.value.bookedDates}");
          log("calender date list: pending: ${dateListModel.value.pendingDates}");
          log("calender date list: unavilable: ${dateListModel.value.unavailableDates}");
  ...
  void onDateClickForEmployee({required DateTime currentDate}) {
    if (currentDate.isBefore(DateTime(DateTime.now().year, DateTime.now().month, DateTime.now().day))) {
  ...
  Future<void> createShortList() async {
    debugPrint('jejeje');
    CustomLoader.show(context!);
    ...
    Either<CustomError, Response> response = await _apiHelper.addToShortlist(addToShortListRequestModel: addToShortListRequestModel);
    CustomLoader.hide(Get.context!);
    response.fold((l) {
      Logcat.msg(l.msg);
    }, (r) {
      if ([200, 201].contains(r.statusCode)) {
        var dataMap= {"selectedShortlist":[(r.body)['details']['_id']]};
        _apiHelper.hireConfirm(dataMap);
        Get.back();
        Utils.showSnackBar(message: (r.body)['message'], isTrue: true);
      // } else {
      //   CustomDialogue.information(
      //     context: Get.context!,
      //     title: MyStrings.error.tr,
      //     description: "${MyStrings.candidate.tr} ${MyStrings.already.tr} ${MyStrings.hired.tr}",
      //   );
      }else {
        Get.back();
        Utils.showSnackBar(message: 'Successfully updated unavailable dates', isTrue: true);
      }
    });
  }
  ...
  void onEmployeeUnavailableDateUpdate({required DateTime currentDate}) {
    if (!appController.user.value.isEmployee) return;
  ...
  void onSameAsStartDatePressedForShortList(bool? value) {
    ...
    onSameAsStartDatePressedForEmployee(value);
  }
