...
class NewCreatePostController extends GetxController {
  ...
  final RxMap<String, Map<String, double>> _fileDimensions = <String, Map<String, double>>{}.obs;
  ...
  Future<void> _getSocialData() async {
    ...
    final images =
        mediaList.where((m) => m.type?.toLowerCase() != 'video').toList();
    if (kDebugMode) {
      print('NewCreatePostController._getSocialData');
    }
    ...
    if (kDebugMode) {
      print('NewCreatePostController._getSocialData 2');
    }
    // Then process videos
    ...
    final int maxConcurrentDownloads = 5; // Adjust for performance
    final client = http.Client();
    final List<Future<void>> downloadTasks = [];

    for (var media in videos) {
      if (downloadTasks.length >= maxConcurrentDownloads) {
        await Future.any(downloadTasks); // Wait for at least one to complete
      }
      downloadTasks.add(_downloadVideo(media, tempDir2, client));
    }

    await Future.wait(
        downloadTasks); // ✅ Ensures all videos download before returning
    client.close();
    if (kDebugMode) {
      print('NewCreatePostController._getSocialData');
    }
  ...
  Future<void> _downloadVideo(
    ...
    try {
      ...
      request.headers['Connection'] = 'Keep-Alive'; // Speed boost
      ...
      if (response.statusCode == 200) {
        ...
        if (kDebugMode) {
          print('❌ 404 Error: Video Not Found -> $mediaUrl');
        }
      } else {
        if (kDebugMode) {
    ...
    try {
      ...
      for (var pickedFile in result!.files) {
        ...
        if (isVideo) {
          ...
          if (kDebugMode) {
            print('Image dimensions: ${dimensions['width']}x${dimensions['height']}');
          }
        }
      }
    ...
    try {
      final post = PostUpload(
        mediaFiles: _selectedFiles.map((file) {
          final dimensions = _fileDimensions[file] ?? {'width': 16, 'height': 9};
  ...
  Future<void> updatePost(BuildContext context) async {
    ...
    try {
      ...
      await BackgroundUploadService.startUpload(
        ...
        postId: socialPost?.id ?? '', // Pass the postId for updating
      );
