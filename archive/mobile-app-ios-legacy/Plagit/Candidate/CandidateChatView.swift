//
//  CandidateChatView.swift
//  Plagit
//
//  Real chat thread backed by GET/POST /candidate/conversations/:id/messages.
//

import SwiftUI

struct CandidateChatView: View {
    ...
    private let currentUserId = CandidateAuthService.shared.currentUserEmail ?? ""
        ...
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                if isLoading {
                    ...
                    Spacer()
                    VStack(spacing: PlagitSpacing.md) {
                        Image(systemName: "bubble.left").font(.system(size: 32)).foregroundColor(.plagitTertiary)
                        Text("No messages yet. Start the conversation!").font(PlagitFont.caption()).foregroundColor(.plagitSecondary)
                    }
                    Spacer()
                } else {
    ...
    private func load() async {
        ...
        do {
            messages = try await CandidateAPIService.shared.fetchMessages(conversationId: conversationId)
        } catch {
    ...
    private var topBar: some View {
        HStack {
            ...
            Spacer()
            Text("Chat").font(PlagitFont.subheadline()).foregroundColor(.plagitCharcoal)
            Spacer()
    ...
    private func messageBubble(_ msg: MessageDTO) -> some View {
        let isMe = msg.senderType == "candidate"
        return HStack {
            if isMe { Spacer() }
            VStack(alignment: isMe ? .trailing : .leading, spacing: PlagitSpacing.xs) {
                if !isMe, let name = msg.senderName {
                    Text(name).font(PlagitFont.micro()).foregroundColor(.plagitTertiary)
                }
                Text(msg.body)
                    .font(PlagitFont.body())
                    .foregroundColor(isMe ? .white : .plagitCharcoal)
                    .padding(.horizontal, PlagitSpacing.lg)
                    .padding(.vertical, PlagitSpacing.md)
                    .background(
                        RoundedRectangle(cornerRadius: PlagitRadius.lg)
                            .fill(isMe ? Color.plagitTeal : Color.plagitCardBackground)
                    )
                ...
                Button { Task { await send() } } label: {
                    if isSending {
                        ...
                        Image(systemName: "arrow.up.circle.fill")
                            ...
                            .foregroundColor(newMessage.trimmingCharacters(in: .whitespaces).isEmpty ? .plagitTertiary : .plagitTeal)
                    }
                }
    ...
    private func send() async {
        ...
        isSending = true
        do {
            let sent = try await CandidateAPIService.shared.sendMessage(conversationId: conversationId, body: text)
