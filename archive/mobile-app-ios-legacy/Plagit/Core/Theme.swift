...
extension Color {
    ...
    // Premium purple — Quick Plug accent
    static let plagitPurple = Color(red: 0.56, green: 0.35, blue: 0.96)
    static let plagitPurpleDark = Color(red: 0.44, green: 0.25, blue: 0.86)
}
...
// MARK: - Reusable Recent Searches Component
...
struct RecentSearchesView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: PlagitSpacing.md) {
            HStack {
                Text("Recent Searches").font(PlagitFont.captionMedium()).foregroundColor(.plagitTertiary)
                Spacer()
                if !searches.isEmpty {
                    Button { withAnimation { searches.removeAll() } } label: {
                        Text("Clear all").font(PlagitFont.captionMedium()).foregroundColor(.plagitTeal)
                    }
                }
            }
            ...
            if searches.isEmpty {
                Text("No recent searches").font(PlagitFont.caption()).foregroundColor(.plagitTertiary)
                    .padding(.horizontal, PlagitSpacing.xl)
            } else {
                    ...
                    HStack(spacing: PlagitSpacing.md) {
                        Button {
                            ...
                            HStack(spacing: PlagitSpacing.sm) {
                                Image(systemName: "clock.arrow.circlepath").font(.system(size: 12, weight: .medium)).foregroundColor(.plagitTertiary)
                                Text(term).font(PlagitFont.body()).foregroundColor(.plagitCharcoal)
                            }
                        }
                        .buttonStyle(.plain)
                        ...
                        Button { withAnimation { searches.removeAll { $0 == term } } } label: {
                            Image(systemName: "xmark").font(.system(size: 10, weight: .medium)).foregroundColor(.plagitTertiary)
                        }
                    }
                    .padding(.horizontal, PlagitSpacing.xl)
    ...
    static func addSearch(_ term: String, to searches: inout [String]) {
